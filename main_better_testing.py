...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for cell in TestCells:
            ...
            num_cells += 1
            val_auroc, val_acc, val_loss = test(model, cell.val_loader, args.no_gpu)

            total_val_auroc += val_auroc
            total_val_acc += val_acc
            total_val_loss += val_loss

            cell.add_valid_auroc(val_auroc, epoch, model.state_dict(), optimizer.state_dict(), args.patience)
        ...
        with open(train_log_fname(args), 'a') as f:
            f.write(f"{epoch},{train_loss},{total_val_loss / num_cells},{total_val_acc / num_cells},{total_val_auroc / num_cells}\n")
    ...
    for cell in TestCells:
        ...
        # Do final testing
        test_auroc, test_acc, test_loss = test(model, cell.test_loader, args.no_gpu)
        data = {
...
class CellDataSet:
    ...
    def __prepare_data_loaders(self):
        dset_val = DeepChromeDataset(
            dataroot=['./dataset/{}/classification/valid.csv'.format(self.cell_id)],
            num_procs=self.dset_workers
        )
        print(f"Validation set has {len(dset_val)} samples.")

        dset_test = DeepChromeDataset(
            dataroot=['./dataset/{}/classification/test.csv'.format(self.cell_id)],
            num_procs=self.dset_workers
        )
